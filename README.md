...
| BT 磁力下载 | 基于 `libtorrent`（libtorrent-rasterbar 官方 Python 绑定），自动处理 DHT、节点发现、分片校验 |
| 超时 / 异常捕获 | 连接 10s、读取 30s 超时；连接中断自动退避重试（默认 5 次），仍失败则标记“出错”并保留已下载分片 |
...
5. 右上角齿轮可设置默认保存目录、单文件线程数（1~16）、同时下载任务数（1~3）、失败重试次数。
